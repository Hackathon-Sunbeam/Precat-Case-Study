...
//void generate_schedule(const char *batch_file, const char *venue_file, const char *faculty_file) ; 
void generate_schedule(const char *batch_file) ; 
...
int main() {
    ...
    load_venues( ); 
    find_venue( );
    //generate_schedule("batch.csv", "venue.csv", "faculties.csv"); 
    generate_schedule("batch.csv");
    return 0;
}
...
void generate_schedule(const char *batch_file) 
{
    ...
    // Read batch data from batch.csv
    FILE *batch_fp = fopen(batch_file, "r");
    if (batch_fp == NULL) {
    ...
    while (fscanf(batch_fp, "%s %d %d %d/%d/%d %d/%d/%d %d:%d:%d %d:%d:%d", batch.Name, &batch.mode, &batch.type,
                  &batch.start_date.DD, &batch.start_date.MM, &batch.start_date.YY,
                  &batch.end_date.DD, &batch.end_date.MM, &batch.end_date.YY,
                  &batch.batch_start_time.hr, &batch.batch_start_time.mm, &batch.batch_start_time.sec,
                  &batch.batch_end_time.hr, &batch.batch_end_time.mm, &batch.batch_end_time.sec) == 15) {
        
        // Print the schedule for this batch
        printf("Batch Name: %s\n", batch.Name);
        printf("Batch Mode: %s\n", batch.mode == 1 ? "Offline" : "Online");
        printf("Batch Type: %s\n", batch.type == 1 ? "Regular" : (batch.type == 2 ? "Fast track" : "Crash"));
        printf("Batch Start Date: %02d/%02d/%02d\n", batch.start_date.DD, batch.start_date.MM, batch.start_date.YY);
...
/*
    ...
    schedule.csv => batchname startdate enddate start_time end_time (batch.csv)
              + venue.csv => lecture_venue , Lab_venue
              + faculties.csv => facutly_name , modulename 

      => batchname startdate enddate start_time end_time 
    ==> data comes from batch.csv 
